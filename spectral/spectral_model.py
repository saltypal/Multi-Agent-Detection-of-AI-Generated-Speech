...
class SpectralAgent:
    def __init__(self, model_dir: str | Path):
        """Initialize the Spectral Agent and load all available trained booster models."""
        ...
        # Load XGBoost
        self.xgb_model = None
        self.xgb_path = self.model_dir / "best_spectral_xgb.json"
        ...
        # Load LightGBM
        self.lgb_model = None
        self.lgb_path = self.model_dir / "best_spectral_lgb.txt"
        ...
        # Load CatBoost
        self.cb_model = None
        self.cb_path = self.model_dir / "best_spectral_cat.json"
        ...
        print(f"[*] Spectral Agent loaded: XGB={self.xgb_model is not None}, LGB={self.lgb_model is not None}, CB={self.cb_model is not None}")

    def predict_features(self, feature_dict: dict, use_best_only: bool = True) -> float:
        """Predict spoof probability directly. Defaults to using the best-performing booster (XGBoost, EER: 38.96%)."""
    ...
    def predict_with_shap(self, audio_path: str | Path) -> tuple[float, dict[str, float]]:
        ...
        if self.xgb_model:
            try:
                ...
                print(f"[!] Warning calculating Spectral Tree-SHAP: {e}")
