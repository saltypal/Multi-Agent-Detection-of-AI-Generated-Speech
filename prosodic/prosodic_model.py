...
XGBoost-based classifier for prosodic deepfake detection.
Operates on tabular features produced by prosodic_feature_extractor.

Key API:
    train_model(train_df, test_df, out_dir)  → trained model
...
def train_model(
    ...
    """
    Train XGBoost on prosodic features.
    ...
    print(f"[Prosodic] Train: {X_train.shape}  Test: {X_test.shape}")
    ...
    print(f"\n{'='*50}")
    print(f"  Prosodic Model — Test Results")
    print(f"{'='*50}")
    ...
    print(f"[Prosodic] Results saved → {out_dir}")
    return model
